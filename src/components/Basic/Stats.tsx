...
function Stats(props: StatsProps) {
  ...
  return (
    <div className="Stats" style={{ backgroundColor: theme.color.dark, color: theme.palette.text.primary }}>
      {stats.map(stat => <div key={`${item._id}stats-${stat}`} className={`Stats-item Stats-item-${stat}`}><span>{convertKey(stat) + ": "}</span><span style={{ backgroundColor: theme.color.base }} className={"value"}>{item[stat].toString()}</span></div>)}
    </div>
  );
}
